...
- **Settings and every other dialog now open fully on screen**: each of Markdown Midget's dialogs opens with its whole title bar on the screen its window is on, and is never taller or wider than the space the taskbar leaves on that screen, whatever the display scaling and wherever the screen sits. When that space is too short for **Edit ▸ Settings…**, its sections scroll and OK and Cancel stay in view below them, and a dialog that grows while it is open stays on the screen. Windows' own message boxes and its Open and Save dialogs place themselves, as before. Before, with the main window maximised on a screen shorter than the dialog, such as 1920×1080 at 150% scaling, Settings opened with its title bar above the top of the screen, where it couldn't be dragged, and the top of its sections cut off; with the main window not maximised, its OK and Cancel could fall below the bottom of the screen instead.
