...
- **Formatting marks (¶) now show in the Markdown source view too**: with **¶** on, the source view shows **¶** at the end of each line and **→** for tabs. The marks are light gray on a light theme and a dim text colour on a dark one, and they aren't copied, found, printed or saved. The one button turns them on and off in both views. Before, the source view showed no marks.
