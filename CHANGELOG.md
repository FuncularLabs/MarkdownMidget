...
their own type, Secure Markdown, and when Windows' file dialog crashes, the notice saves what it found as a log you can
open. Settings and every other dialog, Help, and the main window when no size has been saved now open fully on screen,
even on a short screen at high display scaling, and "Save your current version as…" encrypts a file you name .mdenc. A
prerelease, so these changes get everyday use before 1.0.0; it includes everything in 1.0.0-rc3.

### Changed

- **File ▸ Open now always offers encrypted documents as their own type**: *Secure Markdown* (`*.mdenc`), second in the list after Markdown and named as in Save As, in Windows' dialog and the built-in file picker alike. Open still starts on Markdown, and *All files* still lists `.mdenc` files too. The setting in **Edit ▸ Settings**, now called **Also list encrypted documents (`*.mdenc`) under Markdown**, only adds them to the Markdown type as well. Before, File ▸ Open had no type of its own for encrypted documents: they showed under All files, and under Markdown only with that setting, then called **Show encrypted documents (`*.mdenc`) in the Open dialog**, turned on.
- **The file-dialog crash notice now saves a log you can open**: when Windows' Open or Save dialog crashes, the notice saves what it found as a text file in `%LocalAppData%\MarkdownMidget\logs`, named for the date and time, such as `file-dialog-crash-2026-09-25-093700.txt`, and shows its path. **Open the log** opens it in your text editor. The log starts with the date and time, then gives the version, the helper's exit code, what Windows recorded, and every add-on found that isn't Microsoft's, with its path. Two crashes in the same second get a log each, and the newest 20 logs are kept: older logs the notice saved are deleted, and nothing else in the folder is touched. If the log can't be saved, the clues show as before, the notice says the log couldn't be saved, and **Copy details** takes the place of **Open the log**. Before, **Copy details** copied the same details, without the date and time, to the clipboard, and nothing was saved.

### Fixed

- **Save your current version as… now encrypts a file you name .mdenc**: when a file changes on disk while you have unsaved edits and you click **Save My Version As…**, a name ending in `.mdenc` saves your version encrypted, as Save As does. For an unencrypted document it asks for a password first; an encrypted document isn't asked, and keeps its own password. Cancel the password and nothing is saved: your edits stay in the window, unsaved. If you then keep editing the saved copy, it stays encrypted with that password. The same rule now works the other way too: for an encrypted document, a name that doesn't end in `.mdenc` shows the warning that the copy can be read by anyone, and if you continue it is saved as readable text, as Save As does. Before, an unencrypted document's version was saved as readable text whatever its name, so a `.mdenc` name gave a readable file that looked encrypted, and an encrypted document's version was saved encrypted whatever its name.
- **Settings and every other dialog now open fully on screen**: each of Markdown Midget's dialogs opens with its whole title bar on the screen its window is on, and no taller or wider than the space the taskbar leaves on that screen, whatever the display scaling and wherever the screen sits. When that space is too short for **Edit ▸ Settings…**, **Register as .md editor…**, **Unregister as .md editor** or the notice shown when Windows' file dialog crashes, the dialog's contents scroll and its buttons stay in view below them, and a dialog that grows while it is open stays on the screen. The built-in file picker can still be resized, maximised and snapped as before, including after it moves to a larger screen. Windows' own message boxes and its Open and Save dialogs place themselves, as before. Before, with the main window maximised on a screen shorter than the dialog, such as 1920×1080 at 150% scaling, Settings opened with its title bar above the top of the screen, where it couldn't be dragged, and the top of its sections cut off; with the main window not maximised, its OK and Cancel could fall below the bottom of the screen instead.
- **The main window and Help now open fully on screen when no size has been saved**: on the first start, with no saved size and position yet, Markdown Midget opens at its usual size where that fits, and otherwise shrinks to fit the screen it opens on, centred and never smaller than its minimum size. The Help window (F1) and the changelog that **Help ▸ What's New** opens, which never use a saved size, do the same each time they open. A saved size and position come back as before. Before, on a screen shorter than the window, such as 1920×1080 at 150% scaling, each opened with most of its title bar above the top of the screen.
...
mode: in dark mode the menus, toolbar, dialogs and title bar turn dark, and the document shows the theme you picked for
that mode. View ▸ Mode can keep the app in light or dark mode instead. Four dark themes are new: Obsidiminutive, Red
Sparks, Red Sparks 2X and Amber Phosphor, and your own theme can set the document's text size. Formatting marks show in the
Markdown source view too, and Cancel in Windows' file dialog now just closes it. A prerelease, so these changes get
everyday use before 1.0.0; it includes everything in 1.0.0-rc2.

### Added

- **The menus, toolbar, status bar, dialogs and title bar now follow Windows light and dark mode**: in dark mode they turn dark, and so does the built-in file picker, in every open window and as soon as Windows changes mode. In light mode they look as before. With a Windows contrast theme on, they keep their light look, so the contrast colours stay in charge. Windows' own message boxes and its Open and Save dialogs stay light, and a toolbar button you point at keeps Windows' own faint blue highlight. Before, they stayed light in dark mode.
- **View ▸ Mode now lets you choose Light, Dark or System**: Light or Dark keeps the menus, toolbar, dialogs, title bar and document theme in that mode whatever Windows is set to. System, the default, follows Windows. The choice is saved, and every open window follows it within about a second; if it can't be saved, the status bar says so and that window uses it for now. A Windows contrast theme still wins over Dark. Version 1.0.0-rc2 and earlier keep neither this choice nor the theme for each mode: once one of them saves your settings, Mode is back on System, and this version then treats the theme picked last as it does on an update from 1.0.0-rc2, keeping it for the mode it suits and starting the other mode on its default, for the source view's own themes too.
- **The document theme now follows Windows light and dark mode**: Markdown Midget remembers one theme for light mode and one for dark mode, and switches between them as soon as Windows changes mode, in every open window. **View ▸ Theme** sets the theme for the mode you're in now, Windows' or the one **View ▸ Mode** sets, and the top of the menu says which mode that is. With **Same Theme for Both Views** off, the source view keeps its own theme for each mode too. Until you pick, light mode uses Midget Solarized and dark mode uses Obsidiminutive. The theme you used before stays for the mode it suits, and the other mode starts on its default. If you used Default, light mode now starts on Midget Solarized: pick Default from **View ▸ Theme** to have it back. A dark theme you used in light mode moves to dark mode: pick it again in light mode if you want it there. With a Windows contrast theme on, the light mode theme is used. Before, one theme showed in both modes.
- **New dark theme: Obsidiminutive**, after the Obsidian style in Notepad++: a slate page with pale grey text, orange headings, violet italic links, green inline code, and bullets and numbers in a light steel blue, the colour of its code properties. Bold text is a soft yellow you can read, where Obsidian's own dark navy bold almost disappears. Text, headings, links, bold, quotes, tables and code all meet WCAG AA contrast (4.5:1) on every background the theme gives them in a Markdown document. The one place they don’t is a table header cell holding a heading or a list, which only raw HTML can write: there the headings measure 3.43:1 and the bullets 3.94:1, and they are recorded that way rather than recoloured, because changing them would change every page to suit a hand-written table. Pick it from **View ▸ Theme**. Your own themes can colour bold too: set `--mdm-strong: <colour>` in your theme's `:root`. Bold keeps the colour of the text around it in every other theme and in any theme that doesn't set it, and a theme's bold colour isn't printed.
- **Your own theme can now set the document's text size, its list markers and its inline code colour**: `--mdm-font-size` is the one size the document derives from — set it to `32px` and body text, all six heading levels, list markers, inline and fenced code, table text and the line numbers (not the small ones beside blank lines) double together, on screen and on paper, with no other rule to write. Most diagrams follow it too: flowcharts and class, state, entity-relationship, mind-map, timeline, kanban, block, architecture, swimlane, railroad and Ishikawa diagrams are drawn at that size, with every box drawn to fit its label, and so are a C4 diagram's title and a git graph's branch names. Every diagram is then fitted to the page width, so most grow less than the text around them: at double size a flowchart's labels come out about 1.4 times as large. Seventeen types keep mermaid's own text sizes: user journey, requirement, radar and event modeling diagrams, because the rest of their drawing doesn't grow with the text, and sequence, Gantt, pie, quadrant, XY, Sankey, packet, treemap, Venn, Wardley, Cynefin, tree view and info diagrams, which size their text in settings of their own. `--mdm-list-marker` colours the bullets and the numbers, in nested lists and lists inside a quote; `--mdm-code-fg` colours the text of `inline code` wherever it appears in the formatted view, while fenced blocks keep their own colour and syntax highlighting. All three are optional, and a theme that leaves one out looks exactly as it did before outside its diagrams, as does a theme that already scaled its text by setting the editor's own `--text-base` and `--text-sm`. The exception is a theme that scaled the page with `html { font-size }`: that no longer changes the document's text size, so set `--mdm-font-size` instead. Inside a diagram, in every theme, a label mermaid draws as HTML text (flowchart, class, state, entity-relationship and mind-map labels among them) now has exactly the size and line spacing mermaid measured it at: a label that wraps onto three or more lines keeps the bottom of its last line, an entity-relationship diagram's relationship labels are no longer larger than their boxes, such labels no longer grow out of their boxes in a theme that scales its text through `--text-base`, and each sits slightly higher, centred where mermaid put it. On paper, inline code keeps print's own colours and **list markers print in the default blue-grey whatever `--mdm-list-marker` says** — a marker colour chosen for a dark page is a smudge on white paper — while the text size is yours, so a large theme prints on more paper. If you have a theme with a `::marker` rule of its own, that rule still prints as you wrote it; moving it to `--mdm-list-marker` changes your printouts to the default marker colour. This version's `sample.css` documents all three, but Markdown Midget writes the sample only into an empty `custom\` themes folder, so a copy you already have stays as it was. Before, the text size, the list markers and the inline code colour came from the editor's own palette and no theme could reach them.
- **Two new dark themes, Red Sparks and Red Sparks 2X**, a palette designed by Joe Sparks (@joesparks on X) for reading at night: every colour it sets for the screen is pure red, with no green or blue at all, because red is the light that costs dark-adapted eyes the least. Two things no theme variable reaches stay blue: the highlight behind selected text and Find matches, and the outline round a rule or picture you click. Headings, links, quotes, tables, code and the syntax colours are all reds a shade apart, the spelling squiggle and the table resize handle are full red, brighter than the body text, so they still stand out, and diagrams are tinted red to match. **Red Sparks 2X** is the same palette with the text twice the size — body text, all six heading levels, list markers, inline and fenced code, table text and the line numbers — for tired eyes or a monitor across the room. Pick either from **View ▸ Theme**. Body text is dimmer than the other themes' on purpose: one hue has nothing brighter to offer, and dimmer is the point after dark. On paper a table keeps the theme's look in its header row, red on dark red, and in its borders, and so does every horizontal rule, in dark reds; the page keeps its black text, a white row stripe, the list-marker and inline-code colours it has always printed in, and the diagram as mermaid drew it. With **Color code blocks** on, as it is by default, code blocks print in print's own colours except twelve kinds of highlighted token, which keep the theme's: variables, regular expressions, symbols, selectors, URLs, what a language marks as important (such as CSS's `!important`), lines a diff adds or removes, doctypes, prologs, CDATA sections and entities. The text prints at the theme's size, so a Red Sparks 2X printout of plain text takes about four times the pages: doubling the type in a column of fixed width halves what fits on a line as well as doubling the line.
- **New dark theme: Amber Phosphor**, a second palette for reading at night from Joe Sparks (@joesparks on X): the amber of an old terminal screen, with no blue in any colour the theme sets for the screen, because blue is the part of the light that does most to keep you awake. Two things no theme variable reaches stay blue: the highlight behind selected text and Find matches, and the outline round a rule or picture you click. Keeping green, which Red Sparks gives up, lets it run much brighter: body text is 7.58:1 on the page, clearing WCAG AAA, and headings, links, quotes, list markers, tables and inline code all clear AA (4.5:1). Code comments (4.03:1) and the placeholder in an empty diagram (3.50:1) are dimmer on purpose. A link you point at gets brighter, the spelling squiggle is a true red, the syntax colours run from red through orange and amber to yellow-green, and diagrams are tinted amber to match. Pick it from **View ▸ Theme**. On paper a table keeps the theme's look in its header row, amber on dark brown, and in its borders, and so does every horizontal rule, in dark amber; the page keeps its black text, a white row stripe, the list-marker and inline-code colours it has always printed in, and the diagram as mermaid drew it. With **Color code blocks** on, as it is by default, code blocks print in print's own colours except twelve kinds of highlighted token, which keep the theme's: variables, regular expressions, symbols, selectors, URLs, what a language marks as important (such as CSS's `!important`), lines a diff adds or removes, doctypes, prologs, CDATA sections and entities. CSS selectors, for one, print in its yellow-green, faint on white paper.
- **Formatting marks (¶) now show in the Markdown source view too**: with **¶** on, the source view shows **¶** at the end of each line, **→** for tabs, and **·** for the spaces that are easy to miss but change what Markdown does: at the end of a line (two there make a line break), two or more in a row inside a line (not the spaces after a list marker, or the padding in a table row that starts with `|`), on a line with nothing else, and in indentation that mixes tabs and spaces or stops between a list item's marker and its text. In code blocks, front matter and an HTML comment that starts a line and runs onto the next, only spaces at the end of a line are marked, and a blockquote's `>` doesn't count as indentation. The marks are the theme's own mark colour, as in the formatted view, and they aren't copied, found, printed or saved. The one button turns them on and off in both views. Before, the source view showed no marks.
- **When Windows' Open or Save dialog crashes, Markdown Midget now says what it found**: the file Windows recorded the crash in, if it recorded one, and the add-ons on your PC that are reported to crash Explorer or programs that load them, such as cloud-sync, PDF, graphics and antivirus add-ons, with a count of the other add-ons it found from outside Microsoft. **Copy details** copies all of it for a bug report, **Open this folder in Explorer** opens the folder the dialog started in so you can see which add-ons are at work there, and **How to find the culprit** opens a guide in your web browser that walks through finding the add-on with NirSoft's free ShellExView. Markdown Midget only looks: it never switches an add-on off. As before, it switches to its built-in file picker and opens it so you can carry on. Before, a message said the dialog had closed unexpectedly and that an Explorer add-on is the usual cause, but named no file or add-on.

### Fixed

- **Dark themes now print with dark headings and links**: with Amber Phosphor, Dracula, GitHub Dark Dimmed, Obsidiminutive, Red Sparks, Red Sparks 2X, or a theme of your own that sets `--mdm-color-scheme: dark`, headings print near-black and links dark blue on the white page, and the page stays white even with **Background graphics** ticked in the print preview. A link in a table's header row prints in the header's text colour, so it stays readable on the dark header, and a colour the document gives a heading or link itself still prints. Default and the light themes print as before. Before, a dark theme's headings and links could print too pale to read on white paper, and with Background graphics ticked the page printed dark.
- **Toolbar tooltips now open below an enlarged mouse pointer**: with the pointer made larger in Windows Settings (Accessibility ▸ Mouse pointer and touch ▸ Size), a toolbar tooltip opens under its button, as far down as the pointer is tall, so the pointer no longer covers its text. With a standard-size pointer, tooltips are unchanged. After you change the size, the next window you open uses it. Before, a tooltip opened just below the pointer, and a larger pointer could sit over its text.
...
- **Cancel in Windows' file dialog now just closes it** (#12): in Open, Save As, Export to PDF, Insert Picture and the dictionary import alike. Before, Cancel was taken for a crash: a message said Windows' file picker had closed unexpectedly, Markdown Midget switched to its built-in picker for good and opened it. If that happened to you, go back to Windows' dialog in **Edit ▸ Settings** by clearing **Always use the built-in file picker**.
- **Line numbers now stay in the margin whatever `position` a theme gives the document's blocks**: while line numbers show, `position` on a paragraph, heading, list, list item, quote, code block or table, and on anything that holds one, is overridden on screen, so a theme's `position: relative` no longer drags the numbers in over the text. That has a cost for a theme that positions a block to place its own decoration there: while line numbers show, an absolutely positioned `::before` or `::after` on those blocks, such as an underline under a heading or a tag on a code block, is placed against the whole document instead, so draw decorations like that with a border, a background or a box-shadow. Printing is unaffected, since line numbers don't print. A `filter`, `transform` or a few other effects on a block can still move the numbers; Help and this version's `sample.css` now say to keep effects on backgrounds and `text-shadow`. The shipped themes look exactly as before. Found by Joe Sparks (@joesparks on X). Before, a theme's `position: relative` on paragraphs moved each paragraph's number 72 pixels to the right, onto its first words, and a list item's up to 136.
