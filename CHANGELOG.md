...
- **View ▸ Mode now lets you choose Light, Dark or System**: Light or Dark keeps the menus, toolbar, dialogs, title bar and document theme in that mode whatever Windows is set to. System, the default, follows Windows. The choice is saved, and every open window follows it within about a second; if it can't be saved, the status bar says so and that window uses it for now. A Windows contrast theme still wins over Dark. Version 1.0.0-rc2 and earlier don't keep the choice: once one of them saves your settings, Mode is back on System.
