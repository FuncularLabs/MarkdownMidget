...
- **Line numbers now stay in the margin whatever `position` a theme gives the document's blocks**: while line numbers show, `position` on a paragraph, heading, list, list item, quote, code block or table, and on anything that holds one, is overridden, so a theme's `position: relative` no longer drags the numbers in over the text. A `filter`, `transform` or a few other effects on a block still can; Help and `sample.css` in your themes folder now say to keep effects on backgrounds and `text-shadow`. The shipped themes look exactly as before. Found by Joe Sparks (@joesparks on X). Before, a theme's `position: relative` on paragraphs moved each paragraph's number 72 pixels to the right, onto its first words, and a list item's up to 136.
