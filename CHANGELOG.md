...
- **Your own theme can now set the document's text size, its list markers and its inline code colour**: `--mdm-font-size` is the one size the document derives from — set it to `32px` and body text, all six heading levels, list markers, inline and fenced code, table text and the line numbers double together, on screen and on paper, with no other rule to write. Diagrams follow it too: flowcharts and class, state, entity-relationship and mind-map diagrams are drawn at that size, with every box drawn to fit its label, while pie charts, sequence diagrams and Gantt charts keep mermaid's own text sizes. `--mdm-list-marker` colours the bullets and the numbers, in nested lists and lists inside a quote; `--mdm-code-fg` colours the text of `inline code` wherever it appears, while fenced blocks keep their own colour and syntax highlighting. All three are optional, and a theme that leaves one out looks exactly as it did before outside its diagrams, as does a theme that already scaled its text by setting the editor's own `--text-base` and `--text-sm`. Inside a diagram, in every theme, a label now has exactly the size and line spacing mermaid drew its box for, so it stays inside the box: a label that wraps onto three or more lines keeps the bottom of its last line, an entity-relationship diagram's relationship labels are no longer larger than their boxes, labels no longer grow out of their boxes in a theme that scales its text through `--text-base`, and every label sits slightly higher, centred where mermaid put it. On paper, inline code keeps print's own colours and **list markers print in the default blue-grey whatever `--mdm-list-marker` says** — a marker colour chosen for a dark page is a smudge on white paper — while the text size is yours, so a large theme prints on more paper. If you have a theme with a `::marker` rule of its own, that rule still prints as you wrote it; moving it to `--mdm-list-marker` changes your printouts to the default marker colour. `sample.css` in your themes folder documents all three. Before, the text size, the list markers and the inline code colour came from the editor's own palette and no theme could reach them.
