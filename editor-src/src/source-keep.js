...
// either side, plus the definitions and footnotes it names. Every block there must read as the node it stands for (a changed one:
// or as its own text alone reads, which is all a full serialisation promises). A window below the first block is read below a
// paragraph, as it stands in the document; at the top of a text a `---` line opens front matter, so a block that would put one
// there is the serialiser's. A window that does not read right is written by the serialiser from the neighbours out, at most twice
// more, and then the whole document is; so is a change whose check would cost more than serialising (a long list edited, say).
// A document whose pieces may not read alone as they do together is written whole once anything changes: one with a definition
...
/**
 * What `doc` saves as: { doc, markdown, recs (line records, as pair takes them), tops, linkDefs, unsafe }, a baseline for the next.
...
export function render(doc, serialize, base, parse) {
  ...
  const whole = { doc, markdown: ser.markdown, recs: ser.recs, linkDefs: [], unsafe: false,
    tops: sp && groups.length === n ? sp.map(([from, to], i) => ({ from, to, line: groups[i][0].line, recs: groups[i] })) : null };
  if (!base || base.unsafe || !whole.tops || !parse) return whole;
  ...
  const where = new Map(src.map((j, i) => [j, i])), keep = src.map((j) => j >= 0), passed = new Set();
  ...
  for (let tries = 0; tries < 3; tries++) {
    ...
    const notes = [];
    doc.forEach((c, _, i) => { if (c.type.name === 'footnote_definition') notes.push({ labels: [label(String(c.attrs.label))], text: texts[i] }); });
    ...
    const texted = windows.map(([a, z]) => {
      let md = a ? '' : sepText(-1);
      for (let i = a; i <= z; i++) md += texts[i] + (i < z || z === n - 1 ? sepText(i) : '');
      return { a, z, md };
    });
    if (texted.reduce((sum, w) => sum + w.md.length, 0) > Math.max(4000, smd.length / 20)) return whole;   // a parse costs several serialisations
    const reads = ({ a, z, md }) => {
    ...
    const failed = texted.filter((w) => !reads(w));
    if (!failed.length) {
      ...
      if (FENCE.test(md) && doc.child(0).type.name !== 'front_matter') return whole;   // only front matter may open the text with a fence
      return { doc, markdown: md, recs, tops, linkDefs: base.linkDefs, unsafe: false };
    }
    let widened = false;
