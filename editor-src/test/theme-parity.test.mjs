...
test('a diagram\'s labels take the size and line height mermaid drew their boxes for, never the paragraph\'s', () => {
  ...
  // editor the vendor's `.milkdown-theme-nord p` reaches that <p> too, with the
  // document's size (--text-base) and its own 1.625 line height: at 32px the text was
  // twice its box, and at any size a label wrapped onto three lines lost the bottom of
  // its last one, which mermaid measured at 1.5. So the label inherits both from the SVG,
  // which is what it did while mermaid measured it.
  ...
  const bundle = readFileSync(
    join(here, '..', '..', 'src', 'MarkdownMidget', 'wwwroot', 'editor.bundle.css'), 'utf8');
  const vendor = declarations(bundle).filter((d) => d.where === '@layer mdm-vendor > .milkdown-theme-nord p');
  ...
  for (const prop of ['font-size', 'line-height']) {
    assert.ok(vendor.some((d) => d.prop === prop), `the premise: the vendor's paragraph rule sets ${prop}`);
  }

  const ours = declarations(read('styles', 'structure.css')).filter((d) => d.where.includes('.mdm-mermaid p'));
  assert.deepEqual(ours.map((d) => [d.where, d.prop, d.value, d.important]), [
    ['.mdm-mermaid p', 'font-size', 'inherit', true],
    ['.mdm-mermaid p', 'line-height', 'inherit', true],
  ]);
  // The label, and not one paragraph of the document around it (theirs have no id), so
  // the baseline test above - which admits these two declarations and no other change -
  // still holds everything outside a diagram to what it was.
  assert.deepEqual(reaches(doc, ours[0].where), ['label']);
  // !important because a theme is a later layer: its own paragraph rule would otherwise
  ...
  // paragraphs, for the same reason in reverse. (Mermaid's own stylesheet sizes no `p`,
  // so there is nothing of mermaid's for either declaration to outrank.)

  // At Default's size the vendor gave a label the same font size mermaid is now handed,
  // so no diagram's text changes size at 16px. Its line height does, deliberately: from
  // the vendor's to mermaid's, which moves each label up by 1/16 of its size, to where
  // mermaid centred it, and gives a wrapped label its last line back.
