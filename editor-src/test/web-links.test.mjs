...
test("Ctrl+click posts openLink with the mark's href, and a plain click in the editable view posts nothing", () => {
  m.roundTrip('[web](https://example.com/a?b=1) [mail](mailto:someone@example.com)');
  ...
  click('mail'); assert.deepEqual(posted, web('mailto:someone@example.com'));
});
test('a relative, file: or javascript: link is refused, though <base> makes its a.href look like a web address', () => {
  m.window.document.head.insertAdjacentHTML('beforeend', '<base href="https://mdm-doc.invalid/">');
  m.roundTrip('[rel](other.md) [file](file:///C:/Windows/win.ini) [js](javascript:alert(1))');
  for (const t of ['rel', 'file', 'js']) { assert.equal(click(t).defaultPrevented, true); assert.deepEqual(posted, [{ type: 'linkRefused' }]); }
});
